  ...
  return (
    <div>
      <div style={s.card}>
        ...
        <div style={s.row}>
          <input value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} style={{ ...s.input, width: 54 }} placeholder="📁" />
      ...
      {channels.map((ch) => (
        <div key={ch.id} style={s.card}>
          {editChannel?.id === ch.id ? (
            <div style={s.row}>
              <input value={editChannel.icon} onChange={(e) => setEditChannel({ ...editChannel, icon: e.target.value })} style={{ ...s.input, width: 54 }} />
            ...
            <div style={{ marginTop: 10, paddingLeft: 16, borderLeft: `3px solid ${ch.color}` }}>
              {ch.subchannels.map((sub) => (
                <div key={sub.id} style={{ ...s.row, marginBottom: 6 }}>
                  {editSub?.id === sub.id ? (
                    <>
                      <input value={editSub.icon} onChange={(e) => setEditSub({ ...editSub, icon: e.target.value })} style={{ ...s.input, width: 44 }} />
              ...
              <div style={{ ...s.row, marginTop: 8 }}>
                <input value={subForm[ch.id]?.icon ?? "💬"} onChange={(e) => setSubForm((p) => ({ ...p, [ch.id]: { ...p[ch.id], icon: e.target.value } }))} style={{ ...s.input, width: 44 }} />
...
function PermissionsTab() {
  ...
  const toggleChannel = async (ch) => {
    ...
    try {
      ...
      if (has) {
        ...
        // También quitar subcanales de este canal
        const subsToRemove = ch.subchannels.map(s => s.id);
        setPerms((p) => ({
          channel_ids: p.channel_ids.filter(id => id !== ch.id),
          subchannel_ids: p.subchannel_ids.filter(id => !subsToRemove.includes(id)),
        }));
      } else {
  ...
  const grantAll = async () => {
    ...
    try {
      for (const ch of channels) {
        if (!perms.channel_ids.includes(ch.id)) {
          await authFetch(`/api/permissions/${selectedUser.id}/channels/${ch.id}`, { method: "POST" });
        }
        for (const sub of ch.subchannels) {
          if (!perms.subchannel_ids.includes(sub.id)) {
            await authFetch(`/api/permissions/${selectedUser.id}/subchannels/${sub.id}`, { method: "POST" });
          }
        }
      }
  ...
  const revokeAll = async () => {
    ...
    try {
      for (const ch of channels) {
        await authFetch(`/api/permissions/${selectedUser.id}/channels/${ch.id}`, { method: "DELETE" }).catch(() => {});
      }
    ...
    <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 14 }}>
      ...
      <div style={s.card}>
        ...
        {users.filter(u => !u.is_admin).length === 0 && (
          <div style={{ color: "#555", fontSize: 13 }}>No hay usuarios no-admin</div>
        )}
      </div>
      ...
      <div>
        {!selectedUser ? (
          <div style={{ ...s.card, color: "#555", fontSize: 14, textAlign: "center", padding: 40 }}>
            ...
            {channels.map((ch) => (
              <div key={ch.id} style={{ marginBottom: 12, background: "#16161e", borderRadius: 10, padding: 12 }}>
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: 8 }}>
                  <input
                    type="checkbox"
                    checked={perms.channel_ids.includes(ch.id)}
                ...
                {/* Subcanales — solo si el canal está activo */}
                  ...
                  <div style={{ paddingLeft: 24, borderLeft: `2px solid ${ch.color}44` }}>
                    {ch.subchannels.map((sub) => (
                      <label key={sub.id} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: 4 }}>
                        <input
                          type="checkbox"
                          checked={perms.subchannel_ids.includes(sub.id)}
                          onChange={() => toggleSubchannel(sub)}
                          disabled={saving}
                        />
                        <span>{sub.icon}</span>
